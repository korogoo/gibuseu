...
백엔드 CS 지식 발표 + 프로젝트 트러블슈팅 공유 스터디. 스터디원 9명이 4일 주기(밤 23:00 고정)로 온라인 발표하고, 조는 같이 듣는 스케줄링 단위일 뿐 전원이 각자 발표한다. 플로우는 D-2 자정 주제 제출 → D-1 연관성 기준 조 배정 → D-day 발표.
...
- `members.yaml`에는 정확히 9명을 채워둬라. 비어있거나 9명이 아니면 먼저 사용자에게 확인해라
